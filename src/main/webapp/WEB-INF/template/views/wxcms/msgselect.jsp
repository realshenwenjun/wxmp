...
<div class="mcontent" style="width:530px;">
            <div style="text-align:left; width:530px;">
				...
				<!-- 
				图文消息news需要预先上传,  后续在素材管理模块，实现上传代码后 再考虑此模块的单用户的图文消息
				 -->
					...
					<div>
						...
						<c:forEach var="media" items="${mediaList}" varStatus="status">
						           <tr style="height:40px;background-color:#f9f9f9;">
						           		<td>
						           		   <c:forEach var="row" items="${newsList}" varStatus="status2">
						           		     <c:if test="${media.mediaId == row.mediaId}">
						           		       <input type="radio" value="${media.mediaId}" name="checkname"/>
						           		     </c:if>
						           		   </c:forEach>
						           		</td>
										<td>
						...
						<table class="fm-table">
							...
							<tbody>
								<c:forEach var="row" items="${textList}" varStatus="status">
								           <tr style="height:40px;background-color:#f9f9f9;">
								           		<td><input type="radio" value="${row.id}" name="radioname"/></td>
												<td>${row.baseId}</td>
												<td>${row.content}</td>
									      </tr>
								</c:forEach>
